...
export default async function StudentLessonPage({
  ...
  const { data: lesson } = await supabase
    .from('lessons')
    .select(`*, lesson_summaries ( * ), vocabulary_items ( * ), homework_items ( * ), lesson_sections ( * ), lesson_attachments ( * )`)
    .eq('id', params.id)
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5 pb-12">
      ...
      {/* ── Vocabulary Audio ─────────────────────────────────────────── */}
      {(lesson as any).voice_file_url && (
        <div className="card p-6" style={{ border: '1px solid rgba(79,70,229,0.12)' }}>
          <h2 className="section-title mb-1">🎙️ Teacher's Audio Review</h2>

          <audio controls className="w-full" src={(lesson as any).voice_file_url} />
        </div>
      )}
      ...
      {summary?.teacher_note && (
        <div className="card p-6" style={{ background: 'linear-gradient(180deg,#ffffff,#f7f4ff)', border: '1px solid rgba(79,70,229,0.12)' }}>
          <h2 className="section-title mb-3">🌟 Teacher's Note</h2>
